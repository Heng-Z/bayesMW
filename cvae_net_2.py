...
class CVAE(nn.Module):
    ...
    def encode_y(self, y):
        ...
        for i in range(self.layer_count):
            ...
            multi_resolution_list.append(y)
        self.multi_resolution_list = multi_resolution_list
    ...
    def decode(self,z,y):
        ...
        y_multi_scale_list = self.multi_resolution_list
        x = F.leaky_relu(z, 0.2)
        # x = torch.cat((x,y4),1)
        x = torch.cat((x,y_multi_scale_list[-1]),1)
        for i in range(1, self.layer_count): # i = 1,2,3,4
            x = getattr(self, "deconv%d" % (i + 1))(x)
            # x =  torch.cat((x,y_list[i]),1)
            x = torch.cat((x,y_multi_scale_list[-i-1]),1)
            if self.batch_norm:
                x = F.leaky_relu(getattr(self, "deconv%d_bn" % (i + 1))(x),0.2)
            else:
                x = F.leaky_relu(x,0.2)
        x = torch.cat((getattr(self, "deconv%d" % (self.layer_count + 1))(x),y),1)
        x = self.batch_norm_final(x)
        x = F.tanh(self.conv_final(x))+y
